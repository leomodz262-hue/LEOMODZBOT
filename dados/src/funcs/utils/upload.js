...
class FileTypeDetector {
  static SIGNATURES = {
    ...
    '00000018': {
      ext: 'mp4',
      mime: 'video/mp4',
      checker: (buffer) => buffer.toString('ascii', 4, 8) === 'ftyp'
    },
    '00000020': {
      ext: 'mp4',
      mime: 'video/mp4',
    ...
    '66747970': {
      ...
      checker: (buffer) => buffer.toString('ascii', 4, 12).includes('qt')
    },
    '574d5656': {
      ext: 'wmv',
      mime: 'video/x-ms-wmv'
...
async function upload(buffer, deleteAfter10Min = false) {
  try {
    ...
    const {
      ext,
      mime
    } = FileTypeDetector.detect(buffer);
    ...
    const {
      download_url,
      sha
    } = await uploader.upload(buffer, filePath);
